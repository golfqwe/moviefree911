...
function page_navigator($before_p,$plus_p,$total,$total_p,$chk_page){   
    ...
    $lt_page=$total_p-4;
    if($chk_page>0){  
        echo "<a  href='tv_cate.php?s_page=$pPrev' class='naviPN'>Prev</a>";
    }
    if($total_p>=11){
        if($chk_page>=4){
            echo "<a $nClass href='tv_cate.php?s_page=0'>1</a><a class='SpaceC'>. . .</a>";   
        }
        if($chk_page<4){
            for($i=0;$i<$total_p;$i++){  
                $nClass=($chk_page==$i)?"class='selectPage'":"";
                if($i<=4){
                echo "<a $nClass href='tv_cate.php?s_page=$i'>".intval($i+1)."</a> ";   
                }
                if($i==$total_p-1 ){ 
                echo "<a class='SpaceC'>. . .</a><a $nClass href='tv_cate.php?s_page=$i'>".intval($i+1)."</a> ";   
                }       
            }
        }
        if($chk_page>=4 && $chk_page<$lt_page){
            ...
            for($i=1;$i<=5;$i++){
                ...
                echo "<a $nClass href='tv_cate.php?s_page=".intval($st_page+$i)."'>".intval($st_page+$i+1)."</a> ";    
            }
            for($i=0;$i<$total_p;$i++){  
                if($i==$total_p-1 ){ 
                $nClass=($chk_page==$i)?"class='selectPage'":"";
        ...
        if($chk_page>=$lt_page){
            for($i=0;$i<=4;$i++){
                ...
                echo "<a $nClass href='tv_cate.php?s_page=".intval($lt_page+$i-1)."'>".intval($lt_page+$i)."</a> ";   
            }
        }        
    }else{
        for($i=0;$i<$total_p;$i++){  
            $nClass=($chk_page==$i)?"class='selectPage'":"";
            echo "<a href='tv_cate.php?s_page=$i' $nClass  >".intval($i+1)."</a> ";   
        }       
    }   
    if($chk_page<$total_p-1){
        echo "<a href='tv_cate.php?s_page=$pNext'  class='naviPN'>Next</a>";
    }
} 
?>
...
$q="SELECT id, name FROM tv_cate  ORDER BY id ASC ";
