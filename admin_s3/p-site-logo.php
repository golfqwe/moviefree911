...
if($file1!=""){
	$type=strrchr($file1,".");
	if(($type==".jpg")||($type==".JPG")||($type==".jpeg")||($type==".JPEG")||($type==".gif")||($type==".GIF")||($type==".png")||($type==".PNG")){
	//del img1
	$part1="../logo-img/$op1";
	@unlink ($part1);
	@copy($tmp1,"../logo-img/$file1");
	
	$sql="UPDATE `site_logo` SET  `site_logo` =  '$file1' WHERE `id` =1 LIMIT 1" or die("ไม่แก้ไขเพิ่มข้อมูลได้ 45");
...
function send_line_notify($message, $token){
			...
			$message = ''.$_SERVER['SERVER_NAME'].' movie56k.com'."\n".' ตรวจสอบถูกต้อง';	
			//$token = $line_token;
