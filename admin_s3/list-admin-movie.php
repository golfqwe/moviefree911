...
if(isset($_GET['del_register'])){
    $result = "delete from register_movie where id='".$_GET['del_register']."'";
    mysqli_query($con_db, $result);
echo "<meta http-equiv='refresh' content='0;url=list-admin-movie.php'>" ; 
   ...
   $query="select * from web_detail where id=1" or die("Error:" . mysqli_error());
...
<table width="980" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <a href="list-api-movie.php" class="w3-tag"style="background-color: #0092ed;">API</a>
                    ...
                    <? }else{?>
                        <a href="#" class="w3-tag" style="background-color: red;" disabled>อัพเดท API</a> 
                    <? } 
                        if($url!=$rpre['host']){
                    ?>
                        <a href="update_movie.php?register=<? echo $url;?>" class="w3-tag">ลงทะเบียน API ดูดหนัง</a>
                    <? } ?> 
...
<a href="https://line.me/ti/p/@dxk4743k"target="_blank"class="w3-tag" style="background-color: red;">เช่าAPI / ขอเลขลงทะเบียน</a>

<? if($rpre['host']!='' and $rpre['register']!='null'){ ?>
รหัสสมาชิก :: <code><? echo $rpre['register'];?></code> <a href="list-admin-movie.php?edit_register=<? echo $rpre['id'];?>"class="w3-tag" style="background-color: green;">แก้ไข </a> <button type="button" class="w3-tag" onClick="window.location='list-admin-movie.php?del_register=<? echo $rpre['id'];?>'" style="background-color: red;">ลบ</button>
                        ...
                        <?} if(isset($_GET['edit_register'])){?>
                            ...
                            <div class="container-fluid">
                                <form action="update_movie.php?up_register=<? echo $_GET['edit_register'];?>" role="form" method="post" enctype="multipart/form-data">
                                    <div class="col-lg-6 form-group">
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="register" placeholder="รหัสสมาชิกอัพเดท API" value="<? echo $rpre['register'];?>" required>
                                            <span class="input-group-addon"><span class="glyphicon glyphicon-asterisk"></span></span>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 form-group">
                                        <input type="submit" name="submit" id="submit" value="ลงทะเบียน API" class="btn btn-success">
                                    </div>
                        ...
                        <? } if($rpre['register']=='null'){ ?>
                            <p>
                            <div class="container-fluid">
                                <form action="update_movie.php?up_register=<? echo $rpre['id'];?>" role="form" method="post" enctype="multipart/form-data">
                                    <div class="col-lg-6 form-group">
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="register" placeholder="รหัสสมาชิกอัพเดท API" required>
                                            <span class="input-group-addon"><span class="glyphicon glyphicon-asterisk"></span></span>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 form-group">
...
$q="SELECT id, cate_id, title, post_date, view, sticky, movie, link_allplayer, link_m3u8 FROM `post` WHERE m_id='$_SESSION[m_id]' and google_drive <> 'series' ";
$q.=" ORDER BY id DESC ";
