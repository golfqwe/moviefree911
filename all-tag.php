...
$cat_tag=$_GET['tag'];
$scate="SELECT * FROM `tag_category` WHERE id='$cat_tag'";
...
<head>
  ...
  <META NAME="keywords" CONTENT="<?=$rcate[1];?>">
  <META NAME="description" CONTENT="<?=$titler[1];?> <?=$rcate[1];?>">
                      ...
                      <tr>
                        <td align="center" valign="middle">
                          ...
                          <a href="<?=$rads2[7];?>" title="<?=$rads2[8];?>" target="_blank">
                            <? if($rads2[2]==1){  ?>
                            <img src="//<?=$titler[3];?>/ads-img/<?=$rads2[9];?>" alt="<?=$rads2[8];?>" width="728"
                              border="0" title="<?=$rads2[8];?>" />
                            <? }else if($rads2[2]==2){ ?>
                ...
                <tr>
                  <td align="center">
                    <table width="730" border="0" align="center" cellpadding="0" cellspacing="0">
                      <tr>
                        <td id="rcorners4" align="center" id="title-movie"
                          style="color: #FFFFFF; font-size: 24px; background-color: #<? echo $theme_bgcolor;?>;">
                          <h1 style="color: #FFFFFF; font-size: 26px; margin-top: 2px;height: 23px;font-weight: 300;">
                            <?=$rcate[1];?></h1>
                        </td>
                      </tr>
                      <tr>
...
function page_navigator($before_p,$plus_p,$total,$total_p,$chk_page){   
    ...
    $lt_page=$total_p-4;
    if($chk_page>0){  
        echo "<a  href='./all-tag.php?s_page=$pPrev&tag=".$_GET['tag']."' class='naviPN'>Prev</a>";
    }
    if($total_p>=11){
        if($chk_page>=4){
            echo "<a $nClass href='./all-tag.php?s_page=0&tag=".$_GET['tag']."'>1</a><a class='SpaceC'>. . .</a>";   
        }
        if($chk_page<4){
            for($i=0;$i<$total_p;$i++){  
                $nClass=($chk_page==$i)?"class='selectPage'":"";
                if($i<=4){
                echo "<a $nClass href='./all-tag.php?s_page=$i&tag=".$_GET['tag']."'>".intval($i+1)."</a> ";   
                }
                if($i==$total_p-1 ){ 
                echo "<a class='SpaceC'>. . .</a><a $nClass href='./all-tag.php?s_page=$i&tag=".$_GET['tag']."'>".intval($i+1)."</a> ";   
                }       
            }
        }
        if($chk_page>=4 && $chk_page<$lt_page){
            ...
            for($i=1;$i<=5;$i++){
                ...
                echo "<a $nClass href='./all-tag.php?s_page=".intval($st_page+$i)."&tag=".$_GET['tag']."'>".intval($st_page+$i+1)."</a> ";    
            }
            for($i=0;$i<$total_p;$i++){  
                if($i==$total_p-1 ){ 
                $nClass=($chk_page==$i)?"class='selectPage'":"";
        ...
        if($chk_page>=$lt_page){
            for($i=0;$i<=4;$i++){
                ...
                echo "<a $nClass href='./all-tag.php?s_page=".intval($lt_page+$i-1)."&tag=".$_GET['tag']."'>".intval($lt_page+$i)."</a> ";   
            }
        }        
    }else{
        for($i=0;$i<$total_p;$i++){  
            $nClass=($chk_page==$i)?"class='selectPage'":"";
            echo "<a href='./all-tag.php?s_page=$i&tag=".$_GET['tag']."' $nClass  >".intval($i+1)."</a> ";   
        }       
    }   
    if($chk_page<$total_p-1){
        echo "<a href='./all-tag.php?s_page=$pNext&tag=".$_GET['tag']."'  class='naviPN'>Next</a>";
    }
} 

$q="SELECT id_movie, category_movie FROM `category_movie` WHERE category_movie='$cat_tag' ORDER BY id DESC ";
...
$before_p=($chk_page*$e_page)+1;
    ...
    $intRows= 0;
    while($objs = mysqli_fetch_row($qr)){
//echo $objs[0];

$qs="SELECT id, title, img, view, movie, projection, imdb  FROM `post` WHERE id='".$objs[0]."' ORDER BY id DESC ";
$qrs=mysqli_query($con_db, $qs);
$objResult = mysqli_fetch_row($qrs);
...
?>
                                ...
                                <div class="box" onmouseover="this.style.backgroundColor='#<? echo $theme_bgcolor;?>';"
                                  ...
                                  <div class="ribbon"><span class="movie-<?=$objResult[4];?>"><?=$objResult[4];?></span>
                                  </div>
                                  <? if(!$objResult[6] == ''){?>
                                  <div class="ribbon2"><span
                                      style="font-size: 12px;margin-left: 3px;margin-top: 1px"><img
                                        src="//<?=$titler[3];?>/img/star.png" width="30%"> <?=$objResult[6];?></span>
                                  </div>
                                  <?}?>
                                    ...
                                    <tr>
                                      <td align="center">
                                        ...
                                        <table width="163" border="0" align="center" cellpadding="0" cellspacing="0">
                                          <tr>
                                            <td align="center">
                                              <table width="163" border="0" align="center" cellpadding="0"
                                                cellspacing="0">
                                                <tr>
                                                  <td align="center"><a
                                                      href="//<?=$titler[3];?>/post-<?=$objResult[0];?>/<?=$posturl;?>.html"
                                                      title="<?=$objResult[1];?>">
                  ...
                  ?>
                                                      <img src="data:image/<?php echo end(explode('-', $objResult[2])); ?>;base64,<?php echo curl($objResult[2]);?>" alt="<?=$objResult[1];?>"
                                                        width="163" height="250" border="0"
                                                        title="<?=$objResult[1];?>" />
                                                      <? }else{ ?>
