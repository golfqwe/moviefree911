...
<p style="background-color: #ff9800;color: #FFFFFF;font-size: 26px; width: 100%;"align="center"> TAG :: <? echo $_GET['tag'];?>
</p>

                <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color: #666666; margin-top: -26px;">

                  <tr>

<?  
  $intRows=0;
  $strSQL="select id, title, img, view, detail, link_youtube, post_date, view, tag_1, tag_2, tag_3, tag_4, tag_5, tag_6, meta_tiele, meta_description, meta_keyword  from `blog`  where tag_1 LIKE '%".$_GET['tag']."%' or tag_2 LIKE '%".$_GET['tag']."%' or tag_3 LIKE '%".$_GET['tag']."%' or tag_4 LIKE '%".$_GET['tag']."%' or tag_5 LIKE '%".$_GET['tag']."%' or tag_6 LIKE '%".$_GET['tag']."%'";
                                      ...
                                      <table width="95%" border="0" cellpadding="5" cellspacing="0">
                                        <tr>
                                          ...
                                          <td width="80%"><a href="//<?=$titler[3];?>/view-<?=$objResult[0];?>/<?=$posturl;?>.html" title="<?=$objResult[1];?>" style="font-size: 19px;"><?=$objResult[1];?></a>
                                            <br> <p>
<? if($objResult[8]!=''){?><a href="tag_blog.php?tag=<?=$objResult[8];?>"><span class="label warning" style="border-radius: 4px;margin:2px;"><?=$objResult[8];?></span></a><?}?>
<? if($objResult[9]!=''){?><a href="tag_blog.php?tag=<?=$objResult[9];?>"><span class="label warning" style="border-radius: 4px;margin:2px;"><?=$objResult[9];?></span></a><?}?>
<? if($objResult[10]!=''){?><a href="tag_blog.php?tag=<?=$objResult[10];?>"><span class="label warning" style="border-radius: 4px;margin:2px;"><?=$objResult[10];?></span></a><?}?>
<? if($objResult[11]!=''){?><a href="tag_blog.php?tag=<?=$objResult[11];?>"><span class="label warning" style="border-radius: 4px;margin:2px;"><?=$objResult[11];?></span></a><?}?>
<? if($objResult[12]!=''){?><a href="tag_blog.php?tag=<?=$objResult[12];?>"><span class="label warning" style="border-radius: 4px;margin:2px;"><?=$objResult[12];?></span></a><?}?>
<? if($objResult[13]!=''){?><a href="tag_blog.php?tag=<?=$objResult[13];?>"><span class="label warning" style="border-radius: 4px;margin:2px;"><?=$objResult[13];?></span></a><?}?>
