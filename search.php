...
<title>ผลการค้นหาหนังออนไลน์ | <?=$titler[1];?></title>
<META NAME="keywords" CONTENT="<?=$titler[6];?>"> 
<META NAME="description" CONTENT="<?=$titler[1];?> ผลการค้นหาหนังออนไลน์ <?=$titler[5];?>">
...
<tr>
                    <td  id="rcorners4" align="center" id="title-movie" style="color: #FFFFFF; font-size: 24px;background-color: #<? echo $theme_bgcolor;?>;"><h1 style="color: #FFFFFF; font-size: 26px; margin-top: 2px;height: 23px;font-weight: 400;">รายการหนังที่ค้นพบ</h1></td>
                  </tr>
                <tr>
                  <td align="center">
...
function page_navigator($before_p,$plus_p,$total,$total_p,$chk_page){   
    ...
    $lt_page=$total_p-4;
    if($chk_page>0){  
        echo "<a  href='?s_page=$pPrev".$urlquery_str."' class='naviPN'>Prev</a>";
    }
    if($total_p>=11){
        if($chk_page>=4){
            echo "<a $nClass href='?s_page=0".$urlquery_str."'>1</a><a class='SpaceC'>. . .</a>";   
        }
        if($chk_page<4){
            for($i=0;$i<$total_p;$i++){  
                $nClass=($chk_page==$i)?"class='selectPage'":"";
                if($i<=4){
                echo "<a $nClass href='?s_page=$i".$urlquery_str."'>".intval($i+1)."</a> ";   
                }
                if($i==$total_p-1 ){ 
                echo "<a class='SpaceC'>. . .</a><a $nClass href='?s_page=$i".$urlquery_str."'>".intval($i+1)."</a> ";   
                }       
            }
        }
        ...
        for($i=0;$i<$total_p;$i++){  
            $nClass=($chk_page==$i)?"class='selectPage'":"";
            echo "<a href='?s_page=$i".$urlquery_str."' $nClass  >".intval($i+1)."</a> ";   
        }       
    }   
    if($chk_page<$total_p-1){
        echo "<a href='?s_page=$pNext".$urlquery_str."'  class='naviPN'>Next</a>";
    }
}	
          
$q="SELECT id, title, img, view, movie, projection, imdb FROM `post` WHERE title LIKE '%$keys%' ";
$q.=" ORDER BY id DESC ";
$qr=mysqli_query($con_db, $q);
$total=mysqli_num_rows($qr);
$e_page=44; // กำหนด จำนวนรายการที่แสดงในแต่ละหน้า   
...
<div class="box" onmouseover="this.style.backgroundColor='#<?=$s_color['color'];?>';" onmouseout="this.style.backgroundColor='#333333';">
 <div class="ribbon"><span class="movie-<?=$objResult[4];?>"><?=$objResult[4];?></span></div>
 <? if(!$objResult[6] == ''){?>
  <div class="ribbon2"><span  style="font-size: 12px;"><img src="//<?=$titler[3];?>/img/star.png" width="30%"> <?=$objResult[4];?></span></div>
<?}?>
